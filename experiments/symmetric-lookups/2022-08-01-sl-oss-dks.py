...
CONFIGS = [
    ...
    IssueConfig('ipdb-lookups-one5', ['--symmetries', 'sym=structural_symmetries(time_bound=0,symmetrical_lookups=one_state, symmetry_rw_length_or_number_states=5)', '--heuristic', 'hipdb=ipdb(max_time=900)', '--search', 'astar(sl_heuristic(component_heuristic=hipdb,symmetries=sym),verbosity=silent)']),
    IssueConfig('ipdb-lookups-sub10', ['--symmetries', 'sym=structural_symmetries(time_bound=0,symmetrical_lookups=subset_of_states, symmetry_rw_length_or_number_states=10)', '--heuristic', 'hipdb=ipdb(max_time=900)', '--search', 'astar(sl_heuristic(component_heuristic=hipdb,symmetries=sym),verbosity=silent)']),
    IssueConfig('ipdb-lookups-all', ['--symmetries', 'sym=structural_symmetries(time_bound=0,symmetrical_lookups=all_states)', '--heuristic', 'hipdb=ipdb()', '--search', 'astar(sl_heuristic(component_heuristic=hipdb,symmetries=sym),verbosity=silent)']),

    # sl + oss/dks
    IssueConfig('ms-oss-noprune-lookups-one5', ['--symmetries', 'sym=structural_symmetries(time_bound=0,symmetrical_lookups=one_state,symmetry_rw_length_or_number_states=5,search_symmetries=oss)', '--heuristic', 'hms=merge_and_shrink(shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[goal_relevance,dfp,total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=50000,threshold_before_merge=1,prune_unreachable_states=false)', '--search', 'astar(sl_heuristic(component_heuristic=hms,symmetries=sym),symmetries=sym,verbosity=silent)']),
    ...
    IssueConfig('ipdb-oss-lookups-one5', ['--symmetries', 'sym=structural_symmetries(time_bound=0,symmetrical_lookups=one_state,symmetry_rw_length_or_number_states=5,search_symmetries=oss)',  '--heuristic', 'hipdb=ipdb(max_time=900)', '--search', 'astar(sl_heuristic(component_heuristic=hipdb,symmetries=sym),symmetries=sym,verbosity=silent)']),
    IssueConfig('ipdb-oss-lookups-sub10', ['--symmetries', 'sym=structural_symmetries(time_bound=0,symmetrical_lookups=subset_of_states,symmetry_rw_length_or_number_states=10,search_symmetries=oss)',  '--heuristic', 'hipdb=ipdb(max_time=900)', '--search', 'astar(sl_heuristic(component_heuristic=hipdb,symmetries=sym),symmetries=sym,verbosity=silent)']),
    IssueConfig('ipdb-oss-lookups-all', ['--symmetries', 'sym=structural_symmetries(time_bound=0,symmetrical_lookups=all_states,search_symmetries=oss)',  '--heuristic', 'hipdb=ipdb(max_time=900)', '--search', 'astar(sl_heuristic(component_heuristic=hipdb,symmetries=sym),symmetries=sym,verbosity=silent)']),
    IssueConfig('ipdb-dks-lookups-one5', ['--symmetries', 'sym=structural_symmetries(time_bound=0,symmetrical_lookups=one_state,symmetry_rw_length_or_number_states=5,search_symmetries=dks)',  '--heuristic', 'hipdb=ipdb(max_time=900)', '--search', 'astar(sl_heuristic(component_heuristic=hipdb,symmetries=sym),symmetries=sym,verbosity=silent)']),
    IssueConfig('ipdb-dks-lookups-sub10', ['--symmetries', 'sym=structural_symmetries(time_bound=0,symmetrical_lookups=subset_of_states,symmetry_rw_length_or_number_states=10,search_symmetries=dks)',  '--heuristic', 'hipdb=ipdb(max_time=900)', '--search', 'astar(sl_heuristic(component_heuristic=hipdb,symmetries=sym),symmetries=sym,verbosity=silent)']),
    IssueConfig('ipdb-dks-lookups-all', ['--symmetries', 'sym=structural_symmetries(time_bound=0,symmetrical_lookups=all_states,search_symmetries=dks)',  '--heuristic', 'hipdb=ipdb(max_time=900)', '--search', 'astar(sl_heuristic(component_heuristic=hipdb,symmetries=sym),symmetries=sym,verbosity=silent)']),
]
...
MIDDLE_REV='2f82df4b86e4c7cae9a49ec2cfa2c1172f87e846'
exp.add_fetcher(
    'data/2022-05-04-sl-oss-dks',
    filter_algorithm=[
        '{}-ms-lookups-one5'.format(MIDDLE_REV),
        '{}-ms-lookups-sub10'.format(MIDDLE_REV),
        '{}-ms-lookups-all'.format(MIDDLE_REV),
        '{}-ms-oss-noprune-lookups-one5'.format(MIDDLE_REV),
        '{}-ms-oss-noprune-lookups-sub10'.format(MIDDLE_REV),
        '{}-ms-oss-noprune-lookups-all'.format(MIDDLE_REV),
        '{}-ipdb-lookups-one5'.format(MIDDLE_REV),
        '{}-ipdb-lookups-sub10'.format(MIDDLE_REV),
        '{}-ipdb-lookups-all'.format(MIDDLE_REV),
        '{}-ipdb-dks-lookups-one5'.format(MIDDLE_REV),
        '{}-ipdb-dks-lookups-sub10'.format(MIDDLE_REV),
        '{}-ipdb-dks-lookups-all'.format(MIDDLE_REV),
        '{}-ipdb-oss-lookups-one5'.format(MIDDLE_REV),
        '{}-ipdb-oss-lookups-sub10'.format(MIDDLE_REV),
        '{}-ipdb-oss-lookups-all'.format(MIDDLE_REV),
    ],
    merge=True
)

name='compare-previous-ms'
outfile = os.path.join(
    exp.eval_dir,
    "%s-%s-%s-%s.html" % (
        exp.name, OLD_REVISION, REVISION, name))
exp.add_report(
    ComparativeReport(
        algorithm_pairs=[
            ('{}-ms-lookups-one5'.format(MIDDLE_REV), '{}-ms-lookups-one5'.format(REVISION)),
            ('{}-ms-lookups-sub10'.format(MIDDLE_REV), '{}-ms-lookups-sub10'.format(REVISION)),
            ('{}-ms-lookups-all'.format(MIDDLE_REV), '{}-ms-lookups-all'.format(REVISION)),
            ('{}-ms-oss-noprune-lookups-one5'.format(MIDDLE_REV), '{}-ms-oss-noprune-lookups-one5'.format(REVISION)),
            ('{}-ms-oss-noprune-lookups-sub10'.format(MIDDLE_REV), '{}-ms-oss-noprune-lookups-sub10'.format(REVISION)),
            ('{}-ms-oss-noprune-lookups-all'.format(MIDDLE_REV), '{}-ms-oss-noprune-lookups-all'.format(REVISION)),
        ],
        attributes=attributes,
    ),
    outfile=outfile,
...
exp.add_report(
    ComparativeReport(
        algorithm_pairs=[
            ('{}-ipdb-lookups-one5'.format(MIDDLE_REV), '{}-ipdb-lookups-one5'.format(REVISION)),
            ('{}-ipdb-lookups-sub10'.format(MIDDLE_REV), '{}-ipdb-lookups-sub10'.format(REVISION)),
            ('{}-ipdb-lookups-all'.format(MIDDLE_REV), '{}-ipdb-lookups-all'.format(REVISION)),
            ('{}-ipdb-dks-lookups-one5'.format(MIDDLE_REV), '{}-ipdb-dks-lookups-one5'.format(REVISION)),
            ('{}-ipdb-dks-lookups-sub10'.format(MIDDLE_REV), '{}-ipdb-dks-lookups-sub10'.format(REVISION)),
            ('{}-ipdb-dks-lookups-all'.format(MIDDLE_REV), '{}-ipdb-dks-lookups-all'.format(REVISION)),
            ('{}-ipdb-oss-lookups-one5'.format(MIDDLE_REV), '{}-ipdb-oss-lookups-one5'.format(REVISION)),
            ('{}-ipdb-oss-lookups-sub10'.format(MIDDLE_REV), '{}-ipdb-oss-lookups-sub10'.format(REVISION)),
            ('{}-ipdb-oss-lookups-all'.format(MIDDLE_REV), '{}-ipdb-oss-lookups-all'.format(REVISION)),
        ],
        attributes=attributes,
    ),
    outfile=outfile,
